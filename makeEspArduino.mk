...
ifeq ($(LIBS),)
  ...
  LIBS += $(shell perl -e 'use File::Find;$$d = shift;while (<>) {$$f{"$$1"} = 1 if /^\s*\#include\s+[<"]([^>"]+)/;}find(sub {print $$File::Find::dir," " if $$f{$$_}}, $$d);' $(USER_LIBS) $(SKETCH))
  ifeq ($(LIBS),)
...
$(BUILD_DIR)/%_.cpp$(OBJ_EXT): %.ino $(BUILD_INFO_H) $(ARDUINO_MK)
	echo  $(<F)
	$(CXX) $(CXX_FLAGS) -x c++ -include $(CORE_DIR)/Arduino.h $< -o $@
...
$(MAIN_EXE): $(CORE_LIB) $(USER_OBJ)
	...
	$(CXX) $(BUILD_INFO_CPP) -o $(BUILD_INFO_OBJ)
	$(LD_COM)
	$(ELF2BIN_COM)
...
print "# Commands\n";
print "CC=$$v{'recipe.c.o.pattern'}\n";
print "CXX=$$v{'recipe.cpp.o.pattern'}\n";
